...
public class FileBackedTaskManager extends InMemoryTaskManager {
    ...
    private String toString(Task task) {
        if (task instanceof Subtask subtask) {
            return String.format("%d,SUBTASK,%s,%s,%s,%d", subtask.getId(), subtask.getTitle(),
                    subtask.getStatus(), subtask.getDescription(), subtask.getEpicId());
        } else if (task instanceof Epic) {
            return String.format("%d,EPIC,%s,%s,%s,", task.getId(), task.getTitle(),
                    task.getStatus(), task.getDescription());
        } else {
            return String.format("%d,TASK,%s,%s,%s,", task.getId(), task.getTitle(),
                    task.getStatus(), task.getDescription());
        }
    }
    ...
    private Task fromString(String value) throws IllegalArgumentException {
        ...
        return switch (type) {
            case TaskType.TASK -> new Task(id, name, description, status);
            case TaskType.EPIC -> new Epic(id, name, description, status);
